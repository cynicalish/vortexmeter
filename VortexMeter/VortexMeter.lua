...
function Player:new(id, reduced, owner)
	local self = {}
	if owner then 
		self.detail = Units[id.." "..owner:GetId()]
	else
...
function Combat:addPlayer(id, owner)
	local unit = nil
	if owner then
		unit = Units[id.." "..owner:GetId()]
	else
		unit = Units[id]
	end
	...
	if not player then
		player = Player:new(id, false, owner)
...
function Combat:linkPetToOwner(pet)
	if not pet.linkedToOwner then
		...
		--cynnical edit
		if not owner then 
			--Print("null owner id: " .. pet.detail.owner.id)
			owner = self:addPlayer(pet.detail.owner.id, nil)
		end
		if owner then
...
local function CombatEventsHandler(info, statType, damageAction)
  ...
  -- Sometimes the API gives us a nil target or caster, ignore these cases.
	...
	if info.owner then
		VortexMeter.overallCombat:addPlayer(info.owner:GetId(), nil)
		VortexMeter.CurrentCombat:addPlayer(info.owner:GetId(), nil)
	end
	
  	local overallCasterInCombat = VortexMeter.overallCombat:addPlayer(info.caster:GetId(), info.owner)
	local casterInCombat = VortexMeter.CurrentCombat:addPlayer(info.caster:GetId(), info.owner)
	...
	if target then
	  local overallTargetInCombat = VortexMeter.overallCombat:addPlayer(info.target:GetId(), nil)
		local targetInCombat = VortexMeter.CurrentCombat:addPlayer(info.target:GetId(), nil)
